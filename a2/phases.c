...
void phase_seven(char* input)
{
    ...
    */
    ...
    append_to_secret_string(7, fortuneList->next->next->next->len + 58);
    for(int i = input[0]; i < input[0]+8; ++i) {
	if(i%2) {
	    fortuneList = fortuneList->next;
	    muteflip();
	    append_to_secret_string(7, i%2 + 58%2);
	}
    }
